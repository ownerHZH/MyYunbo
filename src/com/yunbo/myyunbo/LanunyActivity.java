...
public class LanunyActivity extends AbActivity implements  View.OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		search.setOnClickListener(new View.OnClickListener(){
			...
			public void onClick(View v) {
				...
				if (!AbStrUtil.isEmpty(key)) {
					key=key.trim();
					//if (key.equals("最新电影")) { 
					//	Intent intent = new Intent(LanunyActivity.this,NewMovieActivity.class);  
				//		  startActivity(intent); 
					//}
					//else
						if (key.startsWith("thunder://")
							||key.startsWith("ftp://")||key.startsWith("http://")) {
						DyUtil.urlToPlay=key;
						Intent intent = new Intent();
						intent.setClass(LanunyActivity.this, PlayUrlActivity.class); 
						startActivity(intent);
					}else{
					Bundle bundle = new Bundle();
					bundle.putString("key", key);
					Intent intent = new Intent();
					intent.setClass(LanunyActivity.this, SearchActivity.class);
		...
		( (ImageButton) this.findViewById(R.id.homeButton)).setOnClickListener(new View.OnClickListener() {
			
			@Override
			public void onClick(View v) {
				// TODO Auto-generated method stub
				//Intent intent = new Intent(LanunyActivity.this,HomeActivity.class);  
				//  startActivity(intent);
				finish();
			}
		});;
	        ...
	        if (file.exists()) {
				...
				try {
					in = new ObjectInputStream
					 ...
					 if (!tKeys[0].equals(AbDateUtil.getCurrentDate(AbDateUtil.dateFormatYMD))) {
						 load .start();
					}else {
	...
	public static Thread load=new Thread(new Runnable() {
					...
					public void run() {
						...
						try {
							...
							String[] urls=new String[]{"http://top.baidu.com/buzz?b=26&c=1"
									//,"http://top.baidu.com/buzz?b=4&c=2"
									};
							for (String url : urls) {
	...
	private class LoadKeywordsTask implements Runnable{
		@Override
		public void run() {
			try {if(totalKeys==null)
				readkeys();if(totalKeys==null)
				totalKeys=new String[]{ "露水红颜", "心花路放", "一个人的武林", "游戏", "美女",
						"冒险", "uc", "安卓", "app", "谷歌","多多米","财迷","快播","YY","MSN", };
				key_words = getRandomArray();
